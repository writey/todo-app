<template>
  <div class="todo" :class="{ todo__selected: selected }">
    <div class="todo_head" @click="handleClick">
      <div class="todo_icon" :style="{ color }"><i :class="['fa', `fa-${todo.icon}`]"></i></div>
...
export default {
  ...
  computed: {
    ...
    progressColor () {
      const leftColor = `color-stop(30%, ${this.todo.colors[0]})`;
      const rightColor = `to(${this.todo.colors[1]})`;
      return `-webkit-gradient(linear, left bottom, right bottom, ${leftColor}, ${rightColor})`;
    },
    todayTasks () {
